...
let hmacKey = 'nj9i0gfg5f2jugig6mk8z102d5mmyl1lxcyqh5oc5txfi3lhdexl7fgezepugp5j';
let keyRefreshPromise = null;
let lastKeyRefreshAt = 0;
const KEY_REFRESH_COOLDOWN_MS = 60_000;

async function refreshKey() {
  const now = Date.now();
  if (now - lastKeyRefreshAt < KEY_REFRESH_COOLDOWN_MS) return false;
  if (keyRefreshPromise) return keyRefreshPromise;

  keyRefreshPromise = extractKey()
    .then(newKey => {
      if (newKey && newKey !== hmacKey) {
        hmacKey = newKey;
        lastKeyRefreshAt = Date.now();
        return true;
      }
      return false;
    })
    .catch(() => false)
    .finally(() => { keyRefreshPromise = null; });
...
function buildSignedSearchRequest(paramPairs) {
  ...
  const token = createHmac('sha512', hmacKey).update(message, 'utf8').digest('hex');
...
async function fetchOnce(paramPairs) {
...
async function fetchSearchPageOnce(paramPairs) {
  try {
    return await fetchOnce(paramPairs);
  } catch (err) {
    if (err instanceof AmsApiError && err.status === 401) {
      const refreshed = await refreshKey();
      if (refreshed) return fetchOnce(paramPairs);
    }
    throw err;
  }
}
